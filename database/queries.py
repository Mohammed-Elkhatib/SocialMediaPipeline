...
class SocialMediaQueries:
    ...
    def execute_query(self, query, params=None):
        ...
        try:
            connection = self.db_connection.get_db_connection()
    ...
    def insert_post_and_engagement_metrics(self, platform, post_date, post_time, content, likes, retweets, comments):
        ...
        print(f"Executing insert_post_query with params: {params}")
        ...
        select_post_id_query = """
            ...
            WHERE pf.name = %s AND p.post_date = %s
            ORDER BY p.id DESC LIMIT 1
        """
        params = (platform, post_date)
        print(f"Executing select_post_id_query with params: {params}")
        post = self.execute_query(select_post_id_query, params)

        if post:
            post_id = post[0]['id']
            print(f"Retrieved post ID: {post_id}")
        else:
            print("Error: Post ID retrieval failed.")
            return
        ...
        print("Post and engagement metrics inserted successfully.")
    def insert_word_frequencies(self, word_frequencies):
        ...
        print(f"Word frequencies inserted successfully.")
    ...
    def insert_top_comments(self, comments_data, platform_name):
        ...
        for comment in comments_data:
            ...
            likes = comment['likes']  # Assuming we're using likes to order the top comments
            ...
            if post_id:
                ...
                print(f"Inserted comment for post {post_id} at {post_date} {post_time}")
            else:
                print(f"Error: Post not found for comment '{content}' with post_date {post_date} and post_time {post_time}")
    ...
    def insert_top_likes(self, likes_data, platform_name):
        ...
        for like in likes_data:
            ...
            likes = like['likes']  # Assuming we're using likes to order the top comments
            post_id = self.get_post_id(post_date, post_time, platform_name)
            ...
            if post_id:
                ...
                print(f"Inserted likes for post {post_id} at {post_date} {post_time}")
            else:
                print(f"Error: Post not found for like '{content}' with post_date {post_date} and post_time {post_time}")
