...
class JsonExporter:
    ...
    def export_engagement_data(self, engagement_data: Dict[str, Any]) -> bool:
        ...
        try:
            # Format data specifically for visualization
            dashboard_data = {
                'top_likes': engagement_data.get('top_likes', []),
                'top_comments': engagement_data.get('top_comments', []),
                'top_retweets': engagement_data.get('top_retweets', []),
                'period': engagement_data.get('period', {}),
                'updated_at': datetime.now().isoformat()
            }
    ...
    def export_analysis_results(self, analysis_type: str, data: Dict[str, Any]) -> bool:
        ...
        if analysis_type == 'word_frequency':
            return self.export_word_frequencies(data)
        elif analysis_type == 'engagement':
            return self.export_engagement_data(data)
        else:
